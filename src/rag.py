...
async def get_embeddings(texts: list[str], batch_size: int = 10) -> list[list[float]]:
    ...
    logger.info(
        "POST: `%s` for get embeddings", f"{settings.embeddings_base_url}/embeddings"
    )
...
async def index_document(
    ...
    :param index_name: Индекс в который нужно добавить документы.
    ...
    logger.info("Finished index documents, time %s seconds", elapsed_time)
    return ids
...
async def retrieve_documents(
    ...
    kwargs = {"query_embeddings": await get_embeddings([query])}
